...
Func MoveToPos($_x, $_y, $_z, $_a, $m_range)
	...
	If IsPlayerDead() Or $CheckGameLength = True Or $SkippedMove > 6 Then
		$SkippedMove = 0 ; reset $SkippedMove sinon on ne pourra plus rentrÈ dans la fonction
		$GameFailed = 1
		Return
	EndIf
	Local $toggletry = 0
...
EndFunc   ;==>GetGold
 ...
 Func PauseToSurviveHC() ; fonction qui permet de mettre le jeu en Pause lorsque la vie de votre personnage descend en dessous d'un seuil fixÈ
...
Func BuyPotion()	
	...
	Local $potinstock = Number(FastCheckUiValue('Root.NormalLayer.game_dialog_backgroundScreenPC.game_potion.text', 1, 875)) ; rÈcupÈrÈ les potions en stock
	Local $ClickPotion = Round($NbPotionBuy / 5) ; nombre de clic
	
	If $GameDifficulty = 4 And $NbPotionBuy > 0 Then ; selement si armageddon, NbPotionBuy = 0 on dÈactive la fonction
	   If $potinstock <= $PotionStock Then
		  ...
		  Local $vendortry = 0
		  While IsVendorOpened() = False ; si la fenÍtre n'y est pas
			   ...
			   Else
				  ...
				  $GameFailed = 1
				  Return False  ; si pas fenÍtre on sort de la fonction
			   EndIf
		  WEnd
	   ...
	   EndIf
    Else
	   _Log('Fonction BuyPotion dÈactivÈe')
    EndIf
