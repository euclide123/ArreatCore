...
Func KillMob($name, $offset, $Guid, $test_iterateallobjectslist2)
	...
	If $elite Then
		$CptElite += 1;on compte les Ã©lites
	EndIf
...
;~ 				$source = 5000
		...
		If $buff_table[0] And ($source > $buff_table[4] / $MaximumSource Or $buff_table[5] = "") And (TimerDiff($buff_table[10]) > $buff_table[2] Or $buff_table[2] = "") Then ;skill ActivÃ©
...
Func GoToTown()
	...
	While (IsInTown() = False And IsInMenu() = False)
		...
		If $nbTriesTownPortal < 3 Then
			...
			EndIf
		Else
			; On attack, au cas oÃ¹ il y aurait des mobs aprÃ¨s les fail tp
			Attack()
			LeaveGame()
			$nbTriesTownPortal = 0
			Sleep(10000)
...
Func StashAndRepair()
	...
	If $ToRecycle <> -1 Then
		...
		For $i = 0 To UBound($ToRecycle) - 1
			...
			MouseClick('Left')
			Sleep(Random(40, 250))
			$ItemToRecycle += 1 ;on compste les objets recyclÃ©s
	...
	$GoldByRepaire += $GoldBeforeRepaire - $GoldAfterRepaire;on compte le cout de la réparation
